...
void solve() {
    ...
    for (const auto [a, b] : views::pairwise(s)) {
        if ((a == '*' && b == '*') || (a == '*' && b == '<') || (a == '>' && b == '*')) {
            cout << -1 << endl;
            return;
        }
    ...
    const auto first_r = s.find_first_of('>');
    const auto last_l = s.find_last_of('<');

    if (last_l == string::npos || first_r == string::npos) {
        cout << s.length() << endl;
        return;
    }
    ...
    cout << max(first_r, s.length() - last_l - 1) << endl;
}
